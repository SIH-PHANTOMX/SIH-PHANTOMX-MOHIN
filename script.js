...
function openMessageComposer() {
    ...
    modal.innerHTML = `
        <div style="background: white; padding: 2rem; border-radius: 12px; max-width: 500px; width: 90%;">
            ...
            <textarea placeholder="Type your message..." style="width: 100%; height: 120px; padding: 1rem; border: 1px solid #e9ecef; border-radius: 8px; margin-bottom: 1rem; resize: vertical;"></textarea>
            <div style="display: flex; gap: 1rem; justify-content: flex-end;">
                <button class="btn-secondary" onclick="this.closest('div').parentElement.remove()">Cancel</button>
...
function handleQuickAction(action) {
    switch(action) {
        case 'Send Newsletter':
            showNotification('Newsletter composer opened');
            break;
        case 'Create Event':
            showNotification('Event creation form opened');
            break;
        case 'Add Alumni':
            showNotification('Add alumni form opened');
            break;
        default:
...
function animateStats() {
    ...
    const monitoringValues = document.querySelectorAll('.monitoring-item .value');
    ...
    // Animate monitoring values
    monitoringValues.forEach(value => {
...
function setupScrollAnimations() {
    ...
    document.querySelectorAll('.stats-card, .profile-card, .alumni-item').forEach(el => {
        observer.observe(el);
    });
}
...
function updateStats() {
    setInterval(() => {
        ...
        const engagementValue = document.querySelector('.monitoring-item .value');
        if (engagementValue && engagementValue.textContent.includes('%')) {
            const current = parseInt(engagementValue.textContent);
...
style.textContent = `
    ...
    .profile-card, .stats-card, .monitoring-card {
        transition: all 0.3s ease;
    }
